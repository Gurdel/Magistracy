...
class LogisticRegression:
    ...
    def train(self, data, Olabels, unique_classes):
        ...
        if(num_classes == 2):
            ...
            new_theta, final_cost = self.computeGradient(
                data, local_labels, init_theta)
    ...
    def classify(self, data, Thetas):
        ...
        if(len(Thetas) > 1):
            ...
            cval = np.round(self.sigmoidCalc(np.dot(data, Thetas[0])))+1.0
            return cval
    ...
    def computeCost(self, data, labels, init_theta):
        ...
        if(self.normalization_mode == "l1"):
            regularized_parameter = np.dot(
                llambda/(2*m), np.sum(np.abs(theta2)))
        else:
            regularized_parameter = np.dot(
                llambda/(2*m), np.sum(theta2 * theta2))

        J = (-1.0 / m) * (np.sum(np.log(self.sigmoidCalc(np.dot(data, init_theta))) *
                                 labels + (np.log(1 - self.sigmoidCalc(np.dot(data, init_theta))) * (1 - labels))))
