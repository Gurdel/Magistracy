// Dafny program dz2.dfy compiled into C#
...
// Command Line Options: c:\Users\maksy\Documents\KNU\dz2.dfy /verifyAllModules /spillTargetCode:1 /compile:3 /out:bin\dz2
// dz2.dfy
...
class {:autocontracts} Turnstile {
  ...
  method UserExists(key: int) returns (res: bool)
    ...
    ensures (forall i: int :: 0 <= i < usersCount && users[i] != key) ==> res == false
    decreases key
  {
...
namespace _module {
  ...
  public partial class Turnstile {
    public Turnstile() {
      this.users = new BigInteger[0];
      this.usersCount = BigInteger.Zero;
      this.isOpened = false;
    }
    public  BigInteger[] users {get; set;}
    public  BigInteger usersCount {get; set;}
    public  bool isOpened {get; set;}
    public void __ctor()
    {
      BigInteger[] _nw0 = new BigInteger[Dafny.Helpers.ToIntChecked(Dafny.Helpers.ToIntChecked(new BigInteger(10), "C# arrays may not be larger than the max 32-bit integer"),"C# array size must not be larger than max 32-bit int")];
      BigInteger[] _rhs0 = _nw0;
      BigInteger _rhs1 = BigInteger.Zero;
      bool _rhs2 = false;
      Turnstile _lhs0 = this;
      Turnstile _lhs1 = this;
      Turnstile _lhs2 = this;
      _lhs0.users = _rhs0;
      _lhs1.usersCount = _rhs1;
      _lhs2.isOpened = _rhs2;
    }
    public void AddUser(BigInteger d)
    {
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("AddUser   "));
      bool _0_userExists;
      bool _out0;
      _out0 = (this).UserExists(d);
      _0_userExists = _out0;
      if (!(_0_userExists)) {
        if ((this.usersCount) == (new BigInteger((this.users).Length))) {
          BigInteger[] _1_b;
          BigInteger[] _nw1 = new BigInteger[Dafny.Helpers.ToIntChecked(Dafny.Helpers.ToIntChecked((new BigInteger(2)) * (new BigInteger((this.users).Length)), "C# arrays may not be larger than the max 32-bit integer"),"C# array size must not be larger than max 32-bit int")];
          _1_b = _nw1;
          foreach (BigInteger _guard_loop_0 in Dafny.Helpers.IntegerRange(BigInteger.Zero, this.usersCount)) {
            BigInteger _2_i = (BigInteger)_guard_loop_0;
            if ((true) && (((_2_i).Sign != -1) && ((_2_i) < (this.usersCount)))) {
              (_1_b)[(int)((_2_i))] = (this.users)[(int)(_2_i)];
            }
          }
          (this).users = _1_b;
        }
        var _arr0 = this.users;
        var _index0 = this.usersCount;
        BigInteger _rhs3 = d;
        BigInteger _rhs4 = (this.usersCount) + (BigInteger.One);
        BigInteger[] _lhs3 = this.users;
        BigInteger _lhs4 = this.usersCount;
        Turnstile _lhs5 = this;
        _lhs3[(int)(_lhs4)] = _rhs3;
        _lhs5.usersCount = _rhs4;
      } else {
        Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("  user already exists\n"));
      }
      BigInteger _3_i;
      _3_i = BigInteger.Zero;
      while ((_3_i) < (this.usersCount)) {
        Dafny.Helpers.Print((this.users)[(int)(_3_i)]);
        Dafny.Helpers.Print(Dafny.Sequence<char>.FromString(" "));
        _3_i = (_3_i) + (BigInteger.One);
      }
      Dafny.Helpers.Print(this.usersCount);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
    }
    public bool UserExists(BigInteger key)
    {
      bool res = false;
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("UserExists("));
      Dafny.Helpers.Print(key);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("):"));
      if ((this.usersCount).Sign == 0) {
        res = false;
        Dafny.Helpers.Print(res);
        Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
        return res;
      }
      BigInteger _4_index;
      _4_index = BigInteger.Zero;
      while ((_4_index) < (this.usersCount)) {
        if (((this.users)[(int)(_4_index)]) == (key)) {
          res = true;
          Dafny.Helpers.Print(res);
          Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
          return res;
        }
        _4_index = (_4_index) + (BigInteger.One);
      }
      res = false;
      Dafny.Helpers.Print(res);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
    ...
    public bool ReadUserCard(BigInteger userId)
    {
      bool isUserPermitted = false;
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("ReadUserCard   "));
      if (this.isOpened) {
        Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("   already opened\n"));
        return isUserPermitted;
      }
      bool _out1;
      _out1 = (this).UserExists(userId);
      isUserPermitted = _out1;
      if (isUserPermitted) {
        (this).isOpened = true;
        Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("   opened\n"));
        return isUserPermitted;
      }
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("   wrong user, not opened\n"));
      return isUserPermitted;
    }
    public bool TryGo()
    {
      bool res = false;
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("TryGo    "));
      if (!(this.isOpened)) {
        Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("can't enter - turnstile is closed\n"));
        res = false;
        return res;
      }
      (this).isOpened = false;
      res = true;
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("user entered, now turnstile is closed again\n"));
      return res;
    }
  }
  ...
  public partial class __default {
    ...
    {
      Turnstile _5_turn;
      Turnstile _nw2 = new Turnstile();
      _nw2.__ctor();
      _5_turn = _nw2;
      (_5_turn).AddUser(BigInteger.One);
      (_5_turn).AddUser(new BigInteger(3));
      (_5_turn).AddUser(new BigInteger(3));
      bool _6_buf;
      bool _out2;
      _out2 = (_5_turn).ReadUserCard(BigInteger.One);
      _6_buf = _out2;
      bool _7_res1;
      bool _out3;
      _out3 = (_5_turn).TryGo();
      _7_res1 = _out3;
      Dafny.Helpers.Print(_7_res1);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      bool _8_res2;
      bool _out4;
      _out4 = (_5_turn).TryGo();
      _8_res2 = _out4;
      Dafny.Helpers.Print(_8_res2);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      bool _out5;
      _out5 = (_5_turn).ReadUserCard(new BigInteger(3));
      _6_buf = _out5;
      bool _9_res3;
      bool _out6;
      _out6 = (_5_turn).TryGo();
      _9_res3 = _out6;
      Dafny.Helpers.Print(_9_res3);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
    }
  }
